...
import {FuneralAidRequest} from './funeral-aid.model';

@Component({
  selector: 'app-funeral-aid',
  templateUrl: './funeral-aid.component.html',
  styleUrl: '../forms.scss',
  standalone: false
})
export class FuneralAidComponent extends BaseFormComponent implements OnInit {
  constructor() {
    super();
    this.getRelations();
  }
  ...
  submit() {
    ...
    if (this.form.valid && !this.relatedPersonIDError) {
      const request: FuneralAidRequest = this.form.getRawValue();
